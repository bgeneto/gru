 ...
 * - Ler a lista de argumentos de forma mais inteligente e/ou padronizada
...
/*double energia_particula(long int x) // inserir interacao de segundos vizinhos 
// mudar o parametro de entrada... pedir o número da partícula, ao invés de pedir a posição dela
{
  double ene = 0.0;
  long int xv, i; 
   
  for ( i=-ALCANCE_INT; i<ALCANCE_INT+1; i++ )
  {
    if (!i) continue;
    xv =  (x + i + sis.L )%sis.L;
    if ( sis.r[xv] ) ene +=  ( abs(i) == 1 ) ?  sis.e1 : sis.e2  ;
  }
